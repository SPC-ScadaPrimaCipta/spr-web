...
export default function Layanan() {
  ...
  const cards = layananUtama[activeTab].items.map((item) => {
    const ServiceIcon = designIcons[item.title] || Briefcase;
    return (
      <div
        key={item.title}
        className="bg-slate-950 hover:bg-slate-950/90 border border-slate-800/80 hover:border-amber-500/40 p-6 sm:p-8 rounded-2xl flex flex-col justify-between transition-all duration-300 group shadow-lg relative h-full"
      >
